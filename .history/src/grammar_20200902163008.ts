...
interface LinkNode {
    ...
    _right: LinkNode [];
    left: () => Ast;
    right: (type: TokenType , optional: boolean) => LinkNode [];
    statement: (type: TokenType , optional: boolean) => LinkNode;
...
export class SyntaxTree implements LinkNode {
    ...
    right(type: TokenType, optional: boolean = false) {
        const syntax = new SyntaxTree(type, optional); //this.node(type, optional);
