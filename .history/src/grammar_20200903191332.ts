...
function selectStatement() {
    ...
    const tableName = new Statement(TokenType.FromClause).right(TokenType.Identifier);

    tableName.right(TokenType.OptionalWhereClause, true)
    .right(TokenType.Expression)
    .right(TokenType.StatementTerminator);

    tableName.right(TokenType.OptionalLimitClause, true)
    .right(TokenType.NumberLiteral)
    .right(TokenType.StatementTerminator);

    // Select Grammar
    select.right(TokenType.ColumnList).rightStatement(tableName);
    select.right(TokenType.Identifier).rightStatement(tableName);
    select.right(TokenType.StarIdentifier).rightStatement(tableName);
    select.right(TokenType.Expression).rightStatement(tableName);
