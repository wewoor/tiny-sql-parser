...
import { Token, TokenReg, TokenType } from './definition';
...
export function isInvalidSyntax(statement: string, leftNode: Ast, rightNode: Ast) {
    const grammar = SQLGrammar.get(statement);
    ...
    const existSyntax = getAstNode(grammar, leftNode);
    // console.log('isInvalidSyntax:', existSyntax);
    return !existSyntax;
}
...
export function parser(tokens: Token[]): Ast {
    ...
    const parse = function(prev: Ast): Ast {
        ...
        switch (token.type) {
            case TokenType.SelectStatement: {
                ...
                while (token && token.type !== TokenType.StatementTerminator) {
                    const currentNode = parse(prevNode);
                    prevNode.right?.push(currentNode);
                    // Valid select statement syntax
                    if (isInvalidSyntax(TokenType.SelectStatement, prevNode, currentNode)) {
            ...
            case TokenType.Identifier: {
                ...
                if (prev.type === TokenType.SelectStatement && tokens[current + 1].value === ',') {
                    ...
                    while (token.type !== TokenType.FromClauses) {
